...
class EvenementServiceImplTest {
    ...
    void getAllEvenements_ReturnsList() {
        Evenement e1 = new Evenement(1L, "E1", "D1", LocalDateTime.now().plusDays(1), "L1", 100, 100, "cat", BigDecimal.valueOf(25.5));
        Evenement e2 = new Evenement(2L, "E2", "D2", LocalDateTime.now().plusDays(2), "L2", 50, 50, "cat", BigDecimal.valueOf(30.0));
    ...
    void getEvenementById_ExistingId_ReturnsEvenement() {
        Long id = 1L;
        Evenement evt = new Evenement(id, "E1", "D1", LocalDateTime.now().plusDays(1), "L1", 100, 100, "cat", BigDecimal.valueOf(25.5));
    ...
    @Test
    void deleteEvenement_NonExistingId_ThrowsException() {
        Long id = 1L;
        when(repo.findById(id)).thenReturn(Optional.empty());
        assertThrows(EntityNotFoundException.class, () -> service.deleteEvenement(id));
        verify(repo, times(1)).findById(id);
        verify(repo, never()).deleteById(anyLong()); // ✅ Correction anyInt() → anyLong()
    }
}
